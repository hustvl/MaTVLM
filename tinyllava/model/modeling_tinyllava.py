...
from tinyllava.data import IMAGE_TOKEN_INDEX

from mamba2.hybrid_model import HybridPhiForCausalLM
...
class HybridTinyLlavaForConditionalGeneration(TinyLlavaForConditionalGeneration):
    def __init__(self, config: TinyLlavaConfig):
        super().__init__(config)
        self.language_model = HybridPhiForCausalLM(config.text_config)
    ...
    def generate(
        ...
        **kwargs,
        ...
        if images is not None:
            ...
            image_embeddings = self.encode_images(images).to(self.device)
        else:
