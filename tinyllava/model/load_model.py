...
from .modeling_tinyllava import TinyLlavaForConditionalGeneration, HybridTinyLlavaForConditionalGeneration
...
def load_pretrained_model(model_name_or_path, load_type='hf', load_8bit=False, load_4bit=False, device_map="auto",
    ...
    if model_name_or_path is not None and 'lora' not in model_name_or_path:
        model = HybridTinyLlavaForConditionalGeneration.from_pretrained(model_name_or_path,low_cpu_mem_usage=True,torch_dtype=torch.float16)
        model.language_model = EvalMamba2TransformerHybridModelWrapper.from_pretrained(
                model_name_or_path, torch_dtype=torch.float16, attn_implementation='flash_attention_2'
            )
