...
def merge_projections_for_layers(checkpoint, layer_indices):
    for layer_idx in layer_indices:
        ...
        wqkv_key = f"model.layers.{layer_idx}.attention.wqkv.weight"
        wqkv_key_bias = f"model.layers.{layer_idx}.attention.wqkv.bias"
        wo_key = f"model.layers.{layer_idx}.attention.wo.weight"
        ...
        if wqkv_key in checkpoint:
            ...
            wqkv_weight = checkpoint[wqkv_key]
            ...
            checkpoint[in_proj_key] = wqkv_weight

            # Optionally, remove the old keys to clean up the checkpoint
            del checkpoint[wqkv_key]

        if wqkv_key_bias in checkpoint:
            # Assuming all the projections have the same shape, otherwise adjust accordingly
            wqkv_bias = checkpoint[wqkv_key_bias]

            # Assign the new weight to the corresponding in_proj key
            in_proj_bias_key = f"model.layers.{layer_idx}.mha.in_proj.bias"
            checkpoint[in_proj_bias_key] = wqkv_bias

            # Optionally, remove the old keys to clean up the checkpoint
            del checkpoint[wqkv_key_bias]

        if wo_key in checkpoint:
    ...
    def __init__(self, checkpoint_path, transformer_model, mamba_config, attn_layers, dtype, load_from_hub=False, **kwargs):
        ...
        for layer_idx in range(mamba_config.n_layer):
            if layer_idx in attn_layers:
                layer_encoder = InternLM2MHADecoderLayer(
                    self.config,
                    layer_idx,
                    device="cuda",
                    dtype=dtype,
                ...
                layer_encoder = InternLM2MambaDecoderLayer(
                    mamba_config,
                    layer_idx,
                    device="cuda",
                    dtype=dtype,
