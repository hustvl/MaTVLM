...
synonyms = open('llava_eval/synonyms.txt').readlines()
...
def _get_nouns(lines):
    ...
    nouns = list(set(nouns + [word for (word, pos) in nltk.pos_tag(tokenized) if is_noun(pos)]))
    noun_dict = {}
    if "objects" in nouns:
...
def calculate_accuracy_hallucination(gt_dir, pred_dir, task, gt_file):
    acc_avg_scores = {}
    ...
    gt_labels = _obtain_seg_texts(os.path.join(gt_dir, gt_file))
    preds = extract_conversations(os.path.join(pred_dir, "output_" + task + ".txt"))

    error_file = open(f"{pred_dir}/error_{gt_file.split('.')[0]}.txt", "w")
    ...
    for key in tqdm(gt_labels.keys(), total=len(gt_labels.keys())):
        gt = gt_labels[key]
        pred = preds[key]

        num_nouns_gt, num_nouns_pred = _obtain_nouns(gt, pred)

        json_gt[task][key] = num_nouns_gt
        json_pred[task][key] = num_nouns_pred
        ...
        if (len(acc_scores) > 0  and sum(acc_scores) / len(acc_scores) != 1.0) or (len(hallucination_scores) > 0 and sum(hallucination_scores) / len(hallucination_scores) != 0.0):
            error_file.write(f"Image: {key}\n")
            error_file.write(f"GT: {gt}\n")
            error_file.write(f"GT Nouns: {num_nouns_gt}\n")
            error_file.write(f"Pred: {pred}\n")
            error_file.write(f"Pred Nouns: {num_nouns_pred}\n")
            if len(acc_scores) > 0:
                error_file.write(f"Accuracy: {round((sum(acc_scores) / len(acc_scores))*100, 1)}\n")
            if len(hallucination_scores) > 0:
                error_file.write(f"Hallucination: {round((sum(hallucination_scores) / len(hallucination_scores))*100, 1)}\n")
            error_file.write("--------------------------------------------------\n")
...
if __name__ == "__main__":
    ...
    if os.path.exists(os.path.join(pred_dir, f"results_{args.task}.json")):
        acc_avg_scores, hallucination_avg_scores = json.load(open(os.path.join(pred_dir, f"results_{args.task}.json")))["accuracy"], json.load(open(os.path.join(pred_dir, f"results_{args.task}.json")))["hallucination"]
    else:
        acc_avg_scores, hallucination_avg_scores = calculate_accuracy_hallucination(gt_dir, pred_dir, args.task, args.gt_file)
    ...
    with open(os.path.join(pred_dir, f"results_{args.task}.json"), "w") as f:
        json.dump({"accuracy": acc_avg_scores, "hallucination": hallucination_avg_scores}, f)
