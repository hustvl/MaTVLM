...
def calculate_depth_score(gt_path, pred_path):    
    ...
    for k in tqdm(preds.keys(), total=len(preds.keys())):
        gt = gt_labels[k]
        pred = preds[k]
...
if __name__ == "__main__":
    args = parse_args()
    import os
    if os.path.exists(os.path.join(os.path.dirname(args.pred_path), "depth_scores.txt")):
        with open(os.path.join(os.path.dirname(args.pred_path), "depth_scores.txt")) as f:
            acc_depth_scores = f.readlines()
        acc_depth_scores = [float(s.strip()) for s in acc_depth_scores]
    else:
        acc_depth_scores = calculate_depth_score(args.gt_path, args.pred_path)
        with open(os.path.join(os.path.dirname(args.pred_path), "depth_scores.txt"), "w") as f:
            for score in acc_depth_scores:
                f.write(str(score) + "\n")
