...
class HybridInternLM2Model(InternLM2Model):
    def __init__(self, config: InternLM2Config):
        super().__init__(config)
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if teacher_outputs is not None:
                teacher_hidden_states = teacher_outputs[idx]
                hidden_states_run.append(teacher_hidden_states)

            hidden_states_out = []
            for hidden_states_now in hidden_states_run:
                if self.gradient_checkpointing and self.training and isinstance(decoder_layer, InternLM2DecoderLayer):
            ...
            if teacher_outputs is not None and output_hidden_states and idx != len(self.layers) - 1:
        ...
        if output_hidden_states:
            all_hidden_states += (hidden_states,)
            if teacher_outputs is not None:
                all_teacher_hidden_states += (self.norm(hidden_states_out[1]),)
...
class HybridInternLM2ForCausalLM(InternLM2ForCausalLM):
    """
    Hybrid model for InternLM2. It is a combination of InternLM2 and InternLM2ForCausalLM.
    """

    def __init__(self, config):
        super().__init__(config)
        self.model = HybridInternLM2Model(config)
        self.vocab_size = config.vocab_size
        self.output = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
